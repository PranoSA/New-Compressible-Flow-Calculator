    ...
    function Turbine(iseneff, polyeff, pressure_ratio, work) {
        ...
        this.expandFlow = function (flow) {
            ...
            if (_this.polyeff && _this.work) {
                ...
                var ideal_temp_change = _this.work / flow.Cp;
                // Ideal Pressure Flow 
                var ideal_pressure_ratio = Math.pow((flow.TotalTemp) / (flow.TotalTemp - ideal_temp_change), flow.gamma / (flow.gamma - 1));
                ...
                var actual_output_temp = flow.TotalTemp;
                ...
                var actual_pressure_flow = Flow_1["default"].TPFromTP(flow, flow.TotalPressure / ideal_pressure_ratio);
                var actual_temp_flow = Flow_1["default"].TTFromTT(actual_pressure_flow, actual_output_temp);
            ...
            if (_this.iseneff && _this.work) {
                ...
                //  const pressure_change_ratio = Math.pow((ideal_isen_temp_change+flow.TotalTemp)/(flow.TotalTemp), flow.gamma/(flow.gamma-1));
                var pressure_change_ratio = Math.pow((flow.TotalTemp) / (flow.TotalTemp - ideal_isen_temp_change), flow.gamma / (flow.gamma - 1));
                console.log("Pressure Change Ratio", pressure_change_ratio);
                var output_total_pressure = Flow_1["default"].TPFromTP(flow, flow.TotalPressure / pressure_change_ratio);
